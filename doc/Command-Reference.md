...
| Morphology | MK, (GTRX), ORDERED. WARNING: GTRX (which can also be invoked as GTR) should only be applied to data with non-arbitrary state labels and thus generally should not be used for morphological data. See [Binary and morphological models](Substitution-Models#binary-and-morphological-models) for more details. |
